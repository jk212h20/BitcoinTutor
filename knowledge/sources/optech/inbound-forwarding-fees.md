...
2024
  ...
  LND #8886 updates the BuildRoute function to support inbound forwarding fees
  LND #8627 defaults to rejecting channel settings that require above-zero inbound forwarding fees
  
    LND #6703 and #6934 add support for inbound routing fees
...
Previous Topic:Hardware wallet interface (HWI)




Next Topic:Just-In-Time (JIT) channels
...
- Just-In-Time (JIT) channels (jit-channels)

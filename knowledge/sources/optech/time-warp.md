...
previous 11 blocks, called the median time past (MTP) rule.
...
as selfish mining), can gain precise control over the header time of
...
return to the past, giving the attack its time warp name:
...
until they’re producing six blocks per second, the lower bound allowed by
the MTP rule.
...
protocols that use timelocks.  Since the activation of
...
including eclipse attacks and unwanted
...
A 2018 proposal called forward blocks proposed
...
other transaction data, such as signatures, in extension blocks that
...
A request for solutions was made in 2018 and an
elegant solution was proposed as part of the consensus cleanup soft fork proposal: require the first block in a new
...
2025
  ...
  Consensus cleanup proposal updates time warp grace period to 2 hours
  Bitcoin Core #31376 prevents creating block templates that exploit the timewarp bug
  Discussion about the proposed solution to classic time warp being too stringent


2024

  Discussion about fixing Murch-Zawy time warp in consensus cleanup
  New time warp vulnernability affecting testnet4 despite previous time warp fixes
  Question: how many blocks per second can sustainably be created using a time warp attack?
  Draft BIP for testnet4 includes fix for time warp attack
  Question: where exactly is the ‘off-by-one’ difficulty bug and how does it relate to time warp?


2019

  Consensus cleanup proposal: fix the time warp attack


2018

  Forward blocks: on-chain capacity increases without a hard fork
  Fixing the time warp bug: a discussion
  Requests for soft fork solutions to the time warp attack
...
Previous Topic:Threshold signature




Next Topic:Timelocks
